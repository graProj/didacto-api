...
//TODO :
// 1. JWT 인증 및 인가에 대한 설정을 추가한다.
// 2. Member 엔티티와, 회원 가입 API를 작성하고 데이터베이스에 비밀번호는 암호화하여 저장한다.
// 3. JWT 인증에 대한 로그인 컨트롤러를 작성하고, 인증 과정을 수행하도록 설정한다. 인증 성공 시 클라이언트에게 JWT를 반환한다.
// 4. JWT String 내부 Payload에는 [PK, Email, (이름), 권한(교수자/학생)]이 포함되어야 한다.
