...
import com.didacto.dto.member.MemberQueryResponse;
...
public class MemberService {
    ...
    public List<MemberQueryResponse> queryAll() {
        List<Member> members = memberRepository.findAll();
        return members.stream()
                .map(MemberQueryResponse::toDto)
                .collect(Collectors.toList());
    }

    @Transactional(readOnly = true)
    public MemberQueryResponse query(Long id) {
        Member member = memberRepository.findById(id).orElseThrow(() -> {
            throw new NoSuchElementFoundException404(ErrorDefineCode.MEMBER_NOT_FOUND);
        });

        return MemberQueryResponse.toDto(member);
    }
